...
from aiops.qq_adapter import AdapterConfig, AiopsSessionClient, RecentMessageCache, extract_question, should_accept_event, split_reply
from app.platform_auth import verify_platform_request
...
def test_qq_adapter_service_identity_signature(monkeypatch):
    secret = "shared-service-secret"
    monkeypatch.setenv("AIOPS_INTERNAL_SHARED_SECRET", secret)
    client = AiopsSessionClient(config(aiops_internal_shared_secret=secret))
    payload = {"message": "检查点播故障", "limit": 8}
    body = json.dumps(payload, ensure_ascii=False).encode("utf-8")
    headers = client._platform_headers("/api/fault-kb/chat", body)
    app = Flask(__name__)
    with app.test_request_context("/api/fault-kb/chat", method="POST", data=body, headers={"Content-Type": "application/json", **headers}):
        identity = verify_platform_request(request)
    assert identity is not None
    assert identity.subject == "service:qq-adapter"
    assert identity.permissions == frozenset({"netops.ai_chat.use"})
